...
# Default file directory (changes based on user input in form)
file_dir = '/tmp/'
# Naming convention
...
def to_file(report_name, findings, html):
    ...
    with open(file_dir + report_name, 'w') as csvfile:
    ...
    fdrnj = file_dir + report_name_json
    with open(fdrnj, 'w') as jf:
        json.dump(html, jf)
    '''
    report_name_html = report_name.replace(".csv", ".html")
                                           
    htmlfile = open(file_dir + report_name_html, "w")
    htmlfile.write(html)
    htmlfile.close()
    '''
...
def get_bitbucket_urls(userid, password,base_url, page_size):
    ...
    while not is_last_page:
        response = requests.get(base_url + '/rest/api/1.0/repos?limit=' + str(page_size) + '&start=' + str(next_page_start), auth=HTTPBasicAuth(userid, password))
        print(response.json())
...
def get_gitlab_urls(token, base_url, page_size):
    ...
    if not page_size:
        page_size = '20'
    while not is_last_page:
        response = requests.get(base_url + '/api/v4/projects?simple=true&per_page=' + str(page_size) + '&page=' + str(next_page_start), headers=headers)
        print(response.json())
        response_json = response.json()
...
def write_email_config(server, port, from_address, userid, password):
    config = ConfigParser()
...
def email_report(report_recipients, report_name):
    ...
    with open(file_dir + '/' + report_name,'rb') as file:
...
def home():
    ...
    return render_template('index.html', email_server = settings['server'], email_server_port = settings['port'], email_from = settings['from_address'], email_userid = settings['userid'], email_password = settings['password'])
...
def email_settings():
    ...
    write_email_config(server, port, from_address, userid, password)
...
def scan():
    ...
    file_dir = form['report_directory']
    print(file_dir)
    entropy = False
    ...
    elif button  == 'bitbucket':
        urls = get_bitbucket_urls(userid, password, form['url'], page_size)
    elif button  == 'gitlab':
        urls = get_gitlab_urls(password, form['url'], page_size)
    elif button == 'generic':
        urls = form['git_urls']
    ...
    for repo in urls:
        if repo.strip():
            ...
            file_name = file_dir + '/' + repo_to_file_name(repo)
    ...
    to_file(report_name, all_secrets, a_s)
    ...
    return render_template('results.html', scan_results=a_s, scanned_urls=scanned,  skipped_urls=skipped, report_name=f'{report_name}')

@app.route('/previous_results',methods=['GET', 'POST'])
def previous_results():
    previous_results = sorted(glob.glob(file_dir + '/' + report_name_prefix + '*'),reverse=True)
    links = {}
    for result in previous_results:
        # TODO! This might need to be changed to "/" on NON-WINDOWS computers
...
def download (file):
    ...
    if file.startswith(report_name_prefix):
        if file.endswith('.csv'):
            return send_file(path, as_attachment=True)
        if file.endswith('.json'):
            thefile = open(path, "r")
            a_s = json.load(thefile)

            to_csv = file[:-4] + "csv"
            
            return render_template('previous_result.html', scan_results=a_s, report_name=to_csv)
    return "Not Allowed"
